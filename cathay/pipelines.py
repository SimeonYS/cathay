...
class CathayPipeline:

    # Database setup
    conn = sqlite3.connect('cathay.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cathay`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cathay WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cathay (date, title, link, content)"
